...
interface CompanyEventsProps { currentUser: any; }
...
function EventBadge({ status }: { status: string }) {
    const map: Record<string, string> = { upcoming: 'text-orange-600 border-orange-200 bg-orange-50', active: 'text-green-600 border-green-200 bg-green-50', completed: 'text-blue-600 border-blue-200 bg-blue-50' };
    return <Badge variant="outline" className={map[status] || 'text-gray-600 border-gray-200 bg-gray-50'}>{status}</Badge>;
}

export function CompanyEvents({ currentUser }: CompanyEventsProps) {
    ...
    const handleSave = async () => {
        if (!form.title || !form.startDate || !form.endDate || !form.location) { toast.error('Fill all required fields.'); return; }
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Manage your company events</p>
                </div>
                <Dialog open={isOpen} onOpenChange={(o) => { setIsOpen(o); if (!o) { setForm(emptyForm()); setEditingId(null); } }}>
                    <DialogTrigger asChild>
                        <Button className="bg-gradient-to-br from-blue-600 to-purple-700 text-white border-0"><Plus className="h-4 w-4 mr-2" />New Event</Button>
                    </DialogTrigger>
                    <DialogContent className="sm:max-w-[600px]">
                        ...
                        <ScrollArea className="max-h-[65vh] pr-2">
                            <div className="grid gap-4 py-4">
                                {[{ label: 'Title *', field: 'title', placeholder: 'Event title' }, { label: 'Location *', field: 'location', placeholder: 'Venue / city' }].map(({ label, field, placeholder }) => (
                                    <div key={field} className="grid grid-cols-4 items-center gap-4">
                                        <Label className="text-right text-sm">{label}</Label>
                                        <Input className="col-span-3" placeholder={placeholder} value={(form as any)[field]} onChange={e => setForm(p => ({ ...p, [field]: e.target.value }))} />
                                    </div>
                                ))}
                                {/* Category */}
                                ...
                                {form.isPaid && (
                                    <>
                                        <div className="grid grid-cols-4 items-center gap-4">
                                            ...
                                            <Input className="col-span-3" type="number" value={form.price} onChange={e => setForm(p => ({ ...p, price: Number(e.target.value) }))} />
                                        </div>
                                        <div className="grid grid-cols-4 items-center gap-4">
                                            <Label className="text-right text-sm">Payment URL</Label>
                                            <Input className="col-span-3" placeholder="https://..." value={form.joinUrl} onChange={e => setForm(p => ({ ...p, joinUrl: e.target.value }))} />
                                        </div>
                                    </>
                                )}
                            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="bg-muted p-1 rounded-lg inline-flex">
                    {(['upcoming', 'completed'] as const).map(f => (
                        <Button key={f} variant={filter === f ? 'default' : 'ghost'} size="sm" className={`text-xs ${filter !== f ? 'hover:bg-transparent' : ''}`} onClick={() => setFilter(f)}>
            ...
            <div className="space-y-4">
                                ...
                                <div className="flex flex-col md:flex-row items-start justify-between gap-4">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-2 flex-wrap">
                                            ...
                                            {event.is_paid && <Badge variant="outline" className="text-orange-700 border-orange-200 bg-orange-50">Paid · {event.price} DZD</Badge>}
                                        </div>
                                ...
                                <CardContent className="pt-0">
                                    ...
                                    {event.is_paid && event.join_url && (
                                        <div className="mt-3 flex items-center gap-2 text-sm">
                                            ...
                                            <a href={event.join_url?.match(/^https?:\/\//) ? event.join_url : 'https://' + event.join_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{event.join_url}</a>
                                        </div>
                                    )}
                                </CardContent>
                            )}
                ...
                {filtered.length === 0 && (
                    <Card className="border-0 shadow-sm"><CardContent className="flex items-center justify-center p-12">
                        <div className="text-center space-y-2"><Calendar className="h-12 w-12 mx-auto text-muted-foreground" /><p className="text-muted-foreground font-medium">No {filter} events</p></div>
                    </CardContent></Card>
                )}
            </div>
                            ...
                            <ScrollArea className="max-h-[55vh] pr-4">
                                <div className="space-y-4 text-sm">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        {[{ label: 'Creator', value: selectedEvent.creator_name || '—' }, { label: 'Location', value: selectedEvent.location }, { label: 'Start', value: `${formatDate(selectedEvent.start_date)}${selectedEvent.start_time ? ' at ' + selectedEvent.start_time : ''}` }, { label: 'End', value: `${formatDate(selectedEvent.end_date)}${selectedEvent.end_time ? ' at ' + selectedEvent.end_time : ''}` }, { label: 'Applicants', value: (selectedEvent.registration_count || 0).toString() }, { label: 'Category', value: selectedEvent.category_name || '—' }, { label: 'Type', value: selectedEvent.is_paid ? `Paid – ${selectedEvent.price} DZD` : 'Free' }].map(({ label, value }) => (
