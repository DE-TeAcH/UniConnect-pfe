...
import {
    Calendar, MapPin, Mic, Users, Plus, Trash2, Eye, RefreshCw,
    DollarSign, Link, FileDown, ChevronDown, ChevronUp, ArrowUpDown, ArrowUp, ArrowDown, Loader2
} from 'lucide-react';
...
function EventBadge({ status }: { status: string }) {
    const map: Record<string, string> = {
        upcoming: 'text-orange-600 border-orange-200 bg-orange-50 dark:bg-orange-900/20',
        active: 'text-green-600 border-green-200 bg-green-50 dark:bg-green-900/20',
        completed: 'text-blue-600 border-blue-200 bg-blue-50 dark:bg-blue-900/20',
    };
    return <Badge variant="outline" className={map[status] || 'text-gray-600 border-gray-200 bg-gray-50'}>{status}</Badge>;
}

export function TeacherEvents({ currentUser }: TeacherEventsProps) {
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Manage events you created</p>
                </div>
                    ...
                    <Dialog open={isOpen} onOpenChange={(o) => { setIsOpen(o); if (!o) { setForm(emptyForm()); setEditingId(null); } }}>
                        ...
                        <DialogContent className="sm:max-w-[600px]">
                            <DialogHeader>
                                ...
                                <DialogDescription>Fill in all fields to {editingId ? 'update' : 'publish'} this event.</DialogDescription>
                            </DialogHeader>
                                ...
                                <div className="grid gap-4 py-4">
                                    ...
                                    <div className="grid grid-cols-4 items-center gap-4">
                                        ...
                                        <div className="col-span-3 flex items-center gap-3">
                                            ...
                                            <span className="text-sm text-muted-foreground">{form.isPaid ? 'Yes — participants pay to register' : 'No — free to register'}</span>
                                        </div>
                                    </div>
                                    {form.isPaid && (
                                        <>
                                            <div className="grid grid-cols-4 items-center gap-4">
                                                ...
                                                <Input className="col-span-3" type="number" placeholder="0.00" value={form.price}
                                                    onChange={e => setForm(p => ({ ...p, price: Number(e.target.value) }))} />
                                            </div>
                                            <div className="grid grid-cols-4 items-center gap-4">
                                                <Label className="text-right text-sm">Payment URL</Label>
                                                <Input className="col-span-3" placeholder="https://pay.example.dz/..." value={form.joinUrl}
                ...
                <div className="bg-muted p-1 rounded-lg inline-flex">
                    {(['upcoming', 'completed'] as const).map(f => (
                        <Button key={f} variant={filter === f ? 'default' : 'ghost'} size="sm"
                            className={`text-xs capitalize ${filter !== f ? 'hover:bg-transparent' : ''}`}
                            onClick={() => setFilter(f)}>
                                ...
                                <div className="flex flex-col md:flex-row items-start justify-between gap-4">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            {event.is_paid && <Badge variant="outline" className="text-emerald-700 border-emerald-200 bg-emerald-50">Paid</Badge>}
                                        </div>
                                    ...
                                    <div className="flex gap-2 flex-wrap">
                                        <Button variant="outline" size="sm" onClick={() => toggle(event.id)}>
