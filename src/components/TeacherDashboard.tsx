...
import { GraduationCap, Calendar, Users, Trophy, MapPin, TrendingUp } from 'lucide-react';
...
export function TeacherDashboard({ currentUser }: TeacherDashboardProps) {
    ...
    const stats = [
        {
            title: 'Faculty',
            value: currentUser?.affiliation || currentUser?.faculty || 'N/A',
            icon: GraduationCap,
            color: 'text-emerald-600',
            description: 'Your faculty affiliation',
        },
        {
            ...
            icon: Calendar,
            color: 'text-blue-600',
            description: 'Total events you created',
        },
        {
        ...
        <div className="space-y-8">
            <div>
                ...
                <p className="text-muted-foreground mt-2">
                    Welcome back, <strong>{currentUser?.name}</strong> — {currentUser?.affiliation || currentUser?.faculty || 'N/A'}
                </p>
            </div>
                ...
                <CardContent className="space-y-3">
                    {top3.length > 0 ? top3.map((event: any, index: number) => (
                        <div key={event.id} className="flex items-center gap-4 p-4 bg-muted/50 rounded-xl hover:bg-muted transition-colors">
                            <div className={`flex items-center justify-center w-9 h-9 rounded-full text-sm font-bold ${index === 0 ? 'bg-yellow-100 text-yellow-700' :
